...
using Discord = Dotbot.Gateway.Settings.Discord;

namespace Dotbot.Gateway.UnitTests.Services;
...
public class CustomCommandServiceTests
{
    ...
    public async Task Before()
    {
        ...
        guild1.AddCustomCommand("image1", "777").AddAttachment("file1_on_server1", ".png", "https://example.com/file1.png");
        guild1.AddCustomCommand("textandimage1", "888", "some_more_content_on_server1").AddAttachment("file2_on_server1", ".png", "https://example.com/file2.png");
        var guild2 = new Guild("456", "server2");
        guild1.AddCustomCommand("text1", "999", "some_content_on_server2");
        guild1.AddCustomCommand("image5", "666").AddAttachment("file1_on_server2", ".jpg", "https://sample.com/file1.jpg");
    ...
    public async Task SaveCustomCommandAsync_ExistingCommand_IsOverwritten(string commandName, string? content, bool sendAttachment)
    {
        ulong guildId = 123;
        ulong userId = 777;

        var attachment = sendAttachment ? new Attachment(new JsonAttachment { Url = $"https://example.com/{commandName}.png" }) : null;
        if (sendAttachment)
            _handler
